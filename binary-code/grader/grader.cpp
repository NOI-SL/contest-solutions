...
void run_custom_checker(const TestStruct t_obj,
    ...
    ifstream inputFile(t_obj.testcase_input_path);
    ifstream outputFile(t_obj.testcase_output_path);

    string initString;

    if (!inputFile.is_open()) {
        r_obj.message = "SYSTEM ERROR: Can't read the test-case";
        return;
    }

    getline(inputFile, initString);

    inputFile.close();

    if (!outputFile.is_open()) {
        r_obj.message = "SYSTEM ERROR: Can't read the output";
    }
    ...
    getline(outputFile, countString);
    getline(outputFile, currentStr);
    ...
    if (currentStr != initString) {
        r_obj.message = "Wrong Answer";
        return;
    }

    values.insert(currentStr);
    try {
        previous = stoi(currentStr, nullptr, 2);
    } catch (exception e) {
        r_obj.message = "Wrong Answer";
        return;
    }
    ...
    while (getline(outputFile, currentStr)) {
        i++;
        if (initString.size() != currentStr.size()) {
            r_obj.message = "Wrong Answer";
            return;
        }

        if (values.find(currentStr) != values.end()) {
            r_obj.message = "Wrong Answer";
            return;
        }
        ...
        values.insert(currentStr);
        try {
            current = stoi(currentStr, nullptr, 2);
        } catch (exception e) {
            r_obj.message = "Wrong Answer";
            return;
        }

        if (__builtin_popcount(previous ^ current) != 1) {
            r_obj.message = "Wrong Answer";
            return;
        }
    ...
    r_obj.result = true;
    r_obj.score = (float)(i / pow(2, initString.size()));
    r_obj.message = "Score Factor: " + to_string(r_obj.score);
}
// End of BODY
